(function() {
  ...
  if (!authToken) {
    ...
    if (onboardedStatus === 'true') {
      // User is authenticated and onboarded - check premium access
      const premiumAccess = localStorage.getItem('access-to-premium-content');
      
      if (premiumAccess === 'true') {
        // User has premium access - allow access to current page
        // No redirect needed
      } else {
        // User doesn't have premium access - redirect to plan page
        redirectTo('/plan');
      }
    } else {
