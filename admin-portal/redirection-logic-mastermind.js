(function () {
  ...
  if (window.location.origin.includes("server.wized.com")) {
    ...
    window.Wized = window.Wized || [];
    window.Wized.push((Wized) => {
      const courseId = Wized.data.n.parameter.mastermind_id;

      if (!courseId) {
        // Missing course_id parameter - redirect to all courses
        redirectTo("/dashboard/masterminds-workshops/all-mastermind-workshop");
      }
    });
  } else {
    // Normal environment - check URL parameters directly
    const urlParams = new URLSearchParams(window.location.search);
    const courseId = urlParams.get("mastermind_id");
